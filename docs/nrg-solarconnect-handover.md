...
50. **Scope confirmed expanded, per explicit instruction**: bring everything in `nrg-cover` into NRG SolarConnect too, not just the five originally-scoped GAS projects. Six more tools were found hardcoding their own dedicated GAS backend during the `nrg-cover` deep read (Section — see `docs/system-understanding-2026-08-16.md` §1/§3) that were never fully source-read the way the five primary ones were: **Leads**, **Tasks**, **Client Engagement**, **Reference Finder**, **Havells Quotes**, **Referral Network**. Each needs the same full-source read discipline used for the other five (`docs/system-understanding-2026-08-16.md`'s own method) before schema design starts — right now only their client-side call shapes are known (from reading `nrg-cover`'s HTML), not their actual backend logic, triggers, or data model. Locating and reading these six is the next concrete step, same rigor as before: read every line first, only then design.
