...
**Located via the owner's own `nrg-cover` repo rather than Drive search.** `referral_network.html` (1164 lines) was read in full — a standalone CRM for external referral partners (electrical consultants, MEP, architects, PMCs, structural/civil, plumbing consultants, bath/ceramic shops, builders), with three tabs (contact list, a 30-day visit planner, and a WhatsApp broadcast tool) plus an AI business-card-scan-to-form feature. Its backend is yet another standalone Apps Script Web App — its own `SCRIPT_URL`, distinct from every other tool read this session — that a Drive search by title/content could not locate. Rather than schema-design from guesswork, every column in `referral_contacts`/`referral_visit_log`/`referral_wa_log` traces to a concrete field the frontend actually sends or displays, confirmed by reading every `fetch()` call in the file (`getContacts`, `addContact`/`editContact`, `toggleActive`, `logVisit`, `logWA`).
