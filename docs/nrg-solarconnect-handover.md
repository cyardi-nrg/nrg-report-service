...
45. **New**: design and build the Sales/Service Postgres schema itself — Quote Generator, Sales Follow-up, Service Desk (tickets + AMC), Prospect List, Solar Bill Analyser — as NRG SolarConnect modules, each gated by module_entitlements (Section 78). Full architecture already understood end-to-end, see `docs/system-understanding-2026-08-16.md` and `docs/sales-service-migration-plan.md`. Two live security gaps flagged as worth fixing independent of this build: Quote Generator has no authentication at all today, and a two-stage WhatsApp-approval device auth (already built and working in `nrg-cover`, via `gas-verify.gs`) was never wired into it.
