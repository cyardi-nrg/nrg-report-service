...
46. **New**: mockup screens for the Quote Generator + Sales Follow-up module (Section 79) — not yet built, this round was schema-only given the size of the build. Also still open: real RLS for the row-level "own leads/quotes only unless Owner/Sales Head" rule (Section 68), and deciding the ordinary per-kW discount's server-side cap (currently client-JS-only in the live system, ₹4,000/kW — Section 79 carries the column forward but not the enforcement, which belongs with the RLS design).
