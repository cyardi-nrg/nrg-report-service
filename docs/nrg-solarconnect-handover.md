# NRG SolarConnect – Handover Document
...
This is NOT recreated inside NRG SolarConnect. The existing workbook continues to be used.
...
NRG SolarConnect should not replace this in Version 1. Instead:
...
NRG SolarConnect should answer:
...
The competitive advantage of NRG SolarConnect is not AI extraction.
...
This transforms NRG SolarConnect from a document repository into a continuous engineering and costing improvement system.
...
NRG's real project folders live today under two separate top-level Drive folders — "INDUSTRIAL APPLICATION" (~150+ subfolders) and "SPP" (~150+ subfolders) — which the user will consolidate into one location. Inspected both listings directly, plus the full contents of two real project folders, to ground how NRG SolarConnect should link to what already exists and auto-create folders for new projects.
...
Confirmed directly: NRG does **not** keep a separate material-transaction log today. The BOM workbook's single "Actual Qty Used" / "Actual Rate" columns per line item (Section 19) *are* the only record of what actually happened on a historical project — there is no per-dispatch history to import into `material_transactions` for anything already completed or in progress before NRG SolarConnect goes live.
...
- **The calculation logic itself stays in NRG's private SPP sheet, not reimplemented in NRG SolarConnect.** The sheet keeps doing what it already does correctly (lookup/formula-driven quantities from the same driver inputs it uses today); NRG SolarConnect's job is to write inputs in and read calculated quantities back out, not to own the formulas. This avoids two systems' worth of logic quietly drifting apart, and access control (only NRG's owner can edit the sheet) comes for free from Google's own sharing permissions.
...
This is also a **permissions note to carry into the RLS design (item 4)**: Sales needs *read* access to current stock levels (by material/make) even though they do almost no document processing in NRG SolarConnect itself (Section 01) — a different kind of access than the Inbox-driven roles, worth designing for explicitly rather than assuming Sales only ever touches the occasional payment receipt.
...
1. NRG SolarConnect calls the Sheets API to duplicate the SPP workbook's `Template` tab into a fresh sheet (named something like `gen-<bom_id>` for traceability).
...
4. NRG SolarConnect reads the computed `Qty Required` column back out of the same duplicate and writes it as `bom_items` rows (`extraction_status = 'ai_extracted'`, `boms.origin = 'system_calculated'`), same as any other AI-suggested record — reviewed and confirmed by an engineer, never applied directly.
...
**The fix is role-based visibility, not a data model change.** Every screen/action in NRG SolarConnect maps to one or two roles who actually use it — worth writing down explicitly once, so navigation gets designed against it instead of by feel per screen:
...
Real workflow gap: Quote Comparison assumes quotes already exist. Getting them in the first place today means Purchase manually messaging vendors one at a time. Concrete ask: pick 3-5 vendors from a known list (e.g. 10 solar inverter suppliers), get one ready-made inquiry message, and send it via WhatsApp — directly from NRG SolarConnect.
...
## 73. A Pipeline BOM Needs to Be Editable, and the Owner/Sales/Service Split Runs Deeper Than Section 68 First Drew It — No New Schema

**The size-band-standard estimate is a starting point, not a fixed answer.** Real, confirmed need: even with a standardized driver-based BOM, real projects vary — a longer AC Cable run, a different structure count — and the estimate has to reflect *this* project's real numbers before Owner decides on price, not the generic size-band average. **No new schema** — `bom_items.extraction_status`/`confirmed_by`/`confirmed_at` (Section 19) already has exactly the state a manual edit needs (`'corrected'`), the same mechanism used everywhere else a human overrides an AI-suggested or generated value. Editing a pipeline BOM's quantities is the identical action as correcting any other BOM's, just happening before the order is won instead of after. Because `bom_category_cost`/`bom_panel_vs_rest_cost` (Section 59) compute live from whatever `bom_items.planned_quantity` currently holds, editing a quantity recomputes the estimate automatically — no separate recalculation step, no new view.

**The three-way access split is narrower and more specific than Section 68's first pass.** Confirmed explicitly: pricing on a pipeline project is **Owner-only** — not Owner **and** Sales Head, a correction to how Section 68 first framed it. That pairing was always about a different axis — *which prospects* Sales Head can see (the row-level rule) — and was never meant to extend to *who sees pricing* on any one of them. The two rules just happened to share "Owner" as a member and got conflated in the first pass; they don't actually overlap: whoever manages the pipeline row-visibility (Owner + Sales Head) is not automatically who sees cost figures (Owner alone).

Beneath that, Sales and Service also don't see the *same* reduced view — Service sees more than Sales does:
- **Sales** sees only what Sales uploaded for this prospect (Electricity Bill, the Quotation given, a client-shared Schematic Layout) — never the BOM, never pricing.
- **Service** sees the BOM (quantities, editable) and their own uploads (a structure/panel-count working, a site layout sketch) — never pricing, same "quantities visible, ₹ hidden" boundary an active project's BOM tab already uses (Section 39).
- **Owner** sees everything — both roles' documents, the editable BOM, and full pricing/margin.

**No schema needed for the document half either.** `documents.uploaded_by` (Section 27) already identifies who uploaded what — scoping "Sales sees Sales's uploads, Service sees Service's" is a query against a column that already exists, the same way `sales_person_id` already anchored Section 68's row-level rule. Real per-role enforcement still needs actual RLS (Next Session item 4, same still-open item every access rule in Sections 68-70 defers to) — this section just makes the rule itself more precise than Section 68 first stated it, so whenever that design happens it starts from the right shape.

## 74. Generate a Signed Payment Receipt in One Click (0033)

Real ask: Purchase/Accounts should be able to click a button the moment a payment is confirmed and get a formal, auto-populated receipt to print, sign, and send to the client — not type one up by hand from the payment record. Same "the system produces it, a human adjusts and prints" pattern already established for Delivery Challans and Commissioning Reports (Section 37) — and the same reason it works with almost no new schema: everything a receipt needs to say is already sitting in `payment_receipts` (Section 32) the moment a payment is matched and confirmed — amount, `payment_date`, `payment_method`, `payer_name`, `reference_number`, and, via `financial_obligation_id`, what the payment is actually towards.

**`payment_receipt_documents`** — same shape as `delivery_challans`/`commissioning_reports`: `receipt_number` built from `next_document_number()` (Section 37, already reusable for any new series with zero schema change — `'PR'` here, same function that already serves DC and PO numbering), the linked `payment_receipt_id`, and the printed fields (`received_from`, `payment_method`, `received_date`, `amount`, `towards`) pulled at generation time rather than re-derived live, since a receipt is a fixed, signed artifact once issued — it shouldn't silently change if the underlying obligation record is edited later. `generated_document_id` logs the actual PDF as an ordinary `documents` row, same as every other system-generated document in this schema.

**`unique(payment_receipt_id)`** — deliberate: one signed receipt per confirmed payment, never two. Reprinting a lost copy reuses the same row and the same `receipt_number`; it never generates a second receipt for the same payment. Generation should gate on `payment_receipts.match_status = 'confirmed'` (Section 32) — an AI-suggested, unconfirmed payment match has no business becoming a formal document that leaves the building signed; only confirmed data should ever turn into a real, client-facing artifact, same discipline as every other generated document here. `'payment_receipt'` joins the existing free-text `document_type` reference list (Section 36) — no constraint to change.

---

## 75. A Pipeline BOM Needs Rate as Well as Quantity, Its Missing Line Items Added, and Transportation/Fabricator/Electrical as Overridable Numbers, Not Just the Size-Band Default (0034)

**Quantity alone doesn't answer "how much does this change the cost" — quantity times rate does.** Section 73 made `bom_items.planned_quantity` editable on a pipeline BOM; the real ask goes one step further: Owner needs to see the actual ₹ effect of a change, not just the physical one. **No schema change** — `bom_items.planned_rate` (Section 19) already exists, already carries the identical `extraction_status`/`confirmed_by`/`confirmed_at` correction machinery `planned_quantity` does. The gap was purely that the mockup only exposed an editable control for quantity; rate gets the same treatment, same row, same Save action, and the Line Cost column (qty × rate) makes the effect of either edit visible immediately instead of requiring a mental multiplication.

**Missing line items understate cost more than a wrong quantity ever does — so let Owner add them, don't try to estimate around the gap.** Working through Bansal Ceramics' full BOM surfaced four genuinely missing categories — Lightning Arrestor, MC4 Connectors, Earthing Pits, Misc hardware (nut/bolts/clamps/tape) — all real entries in NRG's own 15-category taxonomy (Section 44), just never added to this particular standardized BOM. **No schema change** here either — `bom_items` already supports inserting an ordinary new row for any category at any time; this was purely a missing "+ Add Line Item" action on the pipeline BOM table specifically, the same action any other BOM already needs when a project's real material list doesn't match the size-band starting point.

**Transportation/Fabricator/Electrical Contractor need their own number for *this* deal, not just the size-band average.** `pipeline_project_margin_estimate` (Section 68) stood in for all three with the benchmark average, which is the right default when no real bills exist yet for an unconfirmed order — but Owner sometimes already has a better number (an actual quote a fabricator gave over the phone, a transporter's known rate for this specific route) and needs it to win over the generic average. This is the one genuine schema gap: `boms.estimated_transportation_cost`/`estimated_fabricator_cost`/`estimated_electrical_contractor_cost` (nullable, header-level fields on `boms`, same pattern as `order_value`/`kw_capacity`, Section 19) — null by default, meaning "no better number yet, use the size-band standard." `pipeline_project_margin_estimate` is recreated with `coalesce(boms.estimated_*_cost, size-band standard)` for each of the three, so an entered override wins the moment it's entered and the benchmark quietly falls back to filling in whichever of the three is still blank. The `has_*_benchmark` flags (Section 64/68) are updated the same way — a manual override now counts as "has a number to work with" exactly like a real size-band data point does, since either one means the line isn't silently defaulting to zero, which is the actual thing those flags exist to catch. `material_cost_estimate` needs no equivalent override column — it already comes straight from the BOM's own line items (`bom_panel_vs_rest_cost`, Section 59), edited quantity and rate included, live; editing `bom_items` directly already *is* the override there.

**What this actually found, on Bansal Ceramics:** with the four missing line items filled in and priced at real qty × rate, the Material subtotal moves to ₹36,55,950 (₹38.08/W). Added to Transportation/Fabricator/Electrical Contractor at their unchanged size-band defaults, the Estimated Total Project Cost comes to ₹43,80,750 (₹45.63/W) — against the ₹42,50,000 Quoted Price, that's an Estimated Margin of **−₹1,30,750 (−3.08%)**, a loss. The same BOM, missing those four items, had shown a healthy +₹5,06,720 (+11.92%) margin. The standardized estimate wasn't wrong about what it included — it was silently incomplete, and the incompleteness alone was large enough to flip a deal from apparently-profitable to a real loss at the quoted price. This is the missing-cost-must-never-silently-read-as-zero principle (Section 64/68) showing up as a concrete, consequential example rather than an abstract flag.

**Service's reduced BOM view now actually drops the price columns, closing a real gap.** Section 73 stated Service sees the BOM's quantities but not pricing; the mockup's pipeline BOM table didn't yet implement that split at the column level, only at the whole-section level (BOM shown or hidden entirely for Service vs. Sales). Fixed in the mockup: Rate and Line Cost columns are hidden specifically in the Service view, quantity and unit stay visible — matching the same quantities-visible/₹-hidden boundary an active project's BOM tab already enforces (Section 39). No schema involved; this was a mockup-only correction to bring the UI in line with a rule Section 73 had already stated but not yet fully built.

## 76. Generated Documents Get NRG's Real Letterhead — No Schema, a Mockup-Only Correction

**The professional format already exists — it just isn't applied consistently.** Checked directly against NRG's own real documents: the Quote/Proposal template already carries a full letterhead (navy header bar, logo, "Quality | Commitment | Value" motto in the footer) and looks genuinely professional. The Debit Note carries a lighter version of the same branding. But the real Delivery Challan and the Tally-generated Sales Invoice NRG actually sends out today have **no logo, no letterhead at all** — plain black-and-white tables. The ask ("make the generated documents more professional") isn't asking for a new design; it's asking for the design NRG already has, applied every time instead of some of the time.

**Every `paper-doc` sample (DC, Material Return Note, BOM, Commissioning Report, PO, Payment Receipt) now carries the same letterhead.** Navy header bar with a logo badge and the real registered address/GSTIN (`989/6, G.I.D.C. Makarpura, Vadodara, Gujarat 390010` / `24AABCN8993K1ZV`, both confirmed off three different real NRG documents — Sales Invoice, Debit Note, Delivery Challan — replacing the placeholder `24XXXXX1234X1ZP` the mockup had been using), the document type/number/date on the right, a gold accent divider, and a footer carrying the same "Quality | Commitment | Value" motto NRG's own Quote and Debit Note already use. A generated document's own system note (e.g. "not valid without signature," "draft until approved") stays, just demoted to a small line under the branded footer rather than being the only thing there.

**The logo itself is a stand-in.** The mockup can't embed the actual logo file provided — a real build should use that asset directly (an `<img>`/inline SVG export of it) rather than the sunburst approximation built here for illustration. Everything else — the letterhead layout, the real address/GSTIN, the motto — is drawn straight from NRG's own documents, not invented.

**No schema change.** This is presentation only — every field a generated document prints (company details, doc number, party, line items, totals) was already coming from existing tables (Section 37, Section 74); the letterhead is a template concern for whatever actually renders the PDF in the real build, not a database one.

---

## 77. Renamed ProjectPulse → NRG SolarConnect, and How People Actually Reach It

**Why the rename.** What started as a Project Intelligence Repository (this whole document) is now the first piece of a larger consolidation: NRG's live Sales/Service stack — five Apps Script tools (Service Desk, Quote Generator, Sales Follow-up, Prospect List, Solar Bill Analyser) plus three Railway microservices, all currently backed by Google Sheets — is going to migrate onto the same Postgres/Next.js foundation this document designs. "ProjectPulse" named only the project/inventory half; it no longer describes what this system is becoming. Renamed to **NRG SolarConnect**. No functional change from this rename alone — every schema decision, every migration, every principle in Sections 1-76 stands exactly as designed. This section (and its file rename, `docs/nrg-solarconnect-handover.md`) is the only artifact of the rename itself.

**How people actually get to it: no new front door.** NRG's team doesn't log into a new app to reach this system. They already have one — `nrg-cover`, a Node/Express app on Railway that's the real login/home screen for the whole company: WhatsApp-based device approval, a per-role permissions matrix, and a row of module pills (Leads, Service, Quotes, Tasks, Follow up, etc.) that each route to wherever that module actually lives, some in Apps Script, some in `nrg-cover` itself. **The decision: NRG SolarConnect becomes one more pill in that same row — "Projects."** Nothing new to log into, nothing new to bookmark; it's `nrg-cover`'s existing home screen with one more button, gated by the same `MATRIX`/`MODULE_URLS` mechanism every other module already uses (concretely: a new `'Projects'` entry in `MODULES`, a row per role in `MATRIX` — Owner sees `A` (all), Sales/Service/Purchase get whatever `Section 39`'s role split already calls for — and one URL in `MODULE_URLS` pointing at wherever the Next.js app actually deploys). This is a config change to `nrg-cover`, not a new auth system — and specifically **not** something to push live casually: `nrg-cover` is a real, currently-in-daily-use production app, so this pill gets added only once NRG SolarConnect itself has something real behind that URL to send people to, same discipline as every other module URL in that config today (blank = "coming soon" until it's ready).

**Mockup updated to match.** Every screen's browser-bar URL now reads `nrg.app / projects / ...` instead of a standalone `projectpulse.nrg.app` — illustrating the "lives under the existing domain, not its own" decision directly, not just describing it.

**This is a separate, larger initiative than this document alone covers.** The Sales/Service migration (five Apps Script tools, two of which — Quote Generator and Sales Follow-up — share one live spreadsheet and must move together) has its own real risk profile: years of client quotes with Google Drive PDF links already sent to real customers, historical proposal/client data that must transfer without loss or mismatch. That work is being tracked as its own migration plan, not folded into this schema document — this section exists to record the naming and navigation decisions that connect the two, not to duplicate that plan here.

## 78. Multi-Tenant Foundation — Built for NRG Now, Sellable Later Without a Rewrite (0035)

**The real ask, stated directly:** get NRG itself live on this as soon as possible — but don't close the door on selling it to other solar EPCs later, tiered by module (a future customer might buy only Service, or Service + Sales, or everything). Explicitly **not** asked for: the actual white-label product (custom branding, a signup flow, billing, an admin console for managing other companies) built out now — there's no second customer yet, and building that machinery speculatively means guessing at requirements nobody has stated. The instruction was precise: add now only what's expensive to add later; build the rest only once it's actually needed.

**Two things are expensive to retrofit and cheap to add now — those are what migration 0035 adds:**

1. **Tenant isolation.** `organizations` (the tenant table) plus an `organization_id` column on the schema's five true anchor tables — `employees`, `partners`, `customers`, `materials`, `projects`. Every other table (`documents`, `boms`, `bom_items`, `material_transactions`, `financial_obligations`, `project_milestones`, and every Sales/Service table still to be built) already reaches one of these five via an existing foreign key, so it doesn't need its own copy of the column — its tenant is "whichever organization its project/customer/material belongs to," derived through the FK it already has, never a second independently-set fact that could quietly disagree with the first. Retrofitting this after real project/customer data exists would mean adding a column to every table *and* backfilling the correct tenant onto every existing row by hand; doing it now, before the Sales/Service tables even exist, costs one migration.

2. **Module entitlements — the literal "tick a module, they pay for it" mechanism.** `module_entitlements` (organization × module_key × tier, `basic`/`advanced`) — a missing row means that module is off for that tenant. `module_key` is free text, the same "no schema change to add a new one" reasoning already used for `bom_items.category` (Section 19) — the Sales/Service modules this design session is about to build each just need one more row here, not a schema change. NRG is seeded with every module at the `advanced` tier, because NRG isn't a customer of this switch — it's the reason the switch exists.

**Deliberately, explicitly not built now:** RLS policies that actually *enforce* tenant isolation (this migration makes writing that policy correct — the column exists everywhere it needs to — it doesn't write the policy itself; still the same open Next Session item 4 as always), per-tenant branding/theming, a public signup flow, billing/Stripe integration, an admin console for managing other organizations' accounts. All of that stays out of scope until there's a real second customer to design it against.

**One real schema correction that falls out of this:** `materials.unique(category, canonical_name)` (Section 19/0002) had to become `unique(organization_id, category, canonical_name)` — "DC Cable 4 sq mm" is a perfectly ordinary category name for any solar EPC, not something that should ever collide across two different companies' data.

## 79. Quote Generator + Sales Follow-up, Rebuilt as One Connected Module (0036)

**Built together, not sequenced, because the coupling is real.** The live Quote Generator and Sales Follow-up are two separately-deployed Apps Script projects that share one Google Sheet with no API boundary between them — Sales Follow-up's entire "what has this salesperson quoted" view is a live, unmediated read of Quote Generator's write target (`docs/system-understanding-2026-08-16.md`, confirmed by reading both projects' full source). Migrating one without the other means either freezing the other mid-flight or building a temporary sync bridge; building their replacement as one migration avoids needing either.

**Gated by two separate `module_entitlements` (Section 78) rows** — `sales_quotes` and `sales_followup` — even though NRG uses both together, because a future tenant might not.

**How a customer relationship is actually modeled, and why it needed three pieces, not one:**
- **`leads`** — a raw inbound inquiry, before anyone has quoted it. Matches the live `Leads` tab's real role: exists before a customer necessarily does.
- **`customer_pipeline`** — the CRM/sales-tracking state (temperature, follow-up status, stage, contact counts) — kept as its own table, **not** columns bolted onto the core `customers` table (Section 19), specifically because a tenant without the `sales_followup` module has no reason for these columns to exist on every customer row at all. This is the first table built under the "modules add tables, they don't modify anchor tables" discipline Section 78 exists to enable.
- **`quotes`** — the actual quote, one row per quote generated. A customer can be quoted many times before any deal happens (or ever); flooding the existing `projects` table with a row per speculative quote would have been wrong. A quote only becomes a real project when it's actually won — that conversion isn't automated here, it's a deliberate human action (same "no silent creation" discipline as project-duplicate prevention, Section 69), wiring through to `projects.status = 'in_talks'`/`'active'` exactly as already designed (Section 68).

**Three real bugs fixed, not carried forward — found by reading the live source in full, not by guessing:**

1. **The single most dangerous latent bug found in the whole system**: Sales Follow-up's CEO-discount path read Quote Generator's spreadsheet by hardcoded column position (`row[54]`, `row[46]`, …), correct only by coincidence, silently wrong the moment either project's column layout changed. A real table with named columns makes this entire bug class structurally impossible — there's no "row[54]" to get wrong.
2. **"Historical" (2020-2021 bulk-imported legacy quotes) and live "Quotes" were two tabs, read two different, independently fragile ways by the two projects** (one used hardcoded positional indices, the other dynamic header lookup) — confirmed to actively diverge if a column ever moved. Here they're the same table, same columns, always; `quotes.imported_from_legacy` keeps the provenance without needing a second shape. A legacy quote with no phone on record just has a null `customer_id`-linked phone instead of needing the live system's synthetic `"NOPH_<name>_<SP>"` key scheme.
3. **Deal Status (per-quote) / Temperature (per-customer) / Follow-up Status (per-customer) were three overlapping vocabularies the live code never reconciled** — quoting a client silently reset their Temperature and blanked their Last Follow-up Date, treating "we quoted them" as equivalent to "we just followed up," whether or not that matched reality. Kept as genuinely separate concepts here too (they answer different real questions — how did *this* quote feel vs. how does the relationship currently stand) but named so the difference is obvious (`quotes.quote_time_temperature` vs. `customer_pipeline.temperature`), and **`quotes` deliberately does not auto-write `customer_pipeline`** — a customer's temperature changes only from a visible, attributable action, never as a side effect of an unrelated quote being generated. Same "nothing changes silently" discipline as `project_margin_completeness` (Section 64) and every access-control decision this session, applied here to CRM state instead of cost state.

**Pricing becomes a real table instead of a scanned spreadsheet.** The live rate engine locates wattage-band columns, brand rows, and KW bands by scanning for specific text markers in specific rows/columns of a sheet — confirmed working today, genuinely clever, but a fragile pattern with no schema behind it. `quote_panel_rates` (keyed to the panel's existing `materials` row — Section 19 already models "Solar Panel 545W · Adani (DCR)" as a material, no new concept needed there) and `quote_kw_band_rates` replace it with ordinary named columns. `quote_panel_rates.rate_per_watt` is explicitly a **selling** rate, kept separate from `bom_items.planned_rate` (a cost figure sourced from purchase history) — the two numbers answer different questions and the schema keeps them from ever being confused with each other.

**The CEO-discount PDF regeneration reuses infrastructure that already exists — no new versioning concept.** Setting `quotes.sp_discount` and regenerating the PDF creates a new `documents` row with `supersedes_document_id` pointing at the prior one (Section 19's existing version/supersedes pattern) and repoints `quotes.generated_document_id` — the exact same "a re-issued document is a new version, never an overwrite" rule the schema already enforces for engineering drawings. The old PDF's link is never touched, moved, or invalidated — it stays exactly as issued, same "never break a link someone already has" discipline the baseline audit (Section 5 of the migration plan) exists to protect.

**Two cached, drift-prone columns become live views.** The live `Clients` tab stored `Times Quoted` and `Latest Quote URL` directly, both able to go stale against the real `Quotes` data (a real, observed risk, not theoretical). `customer_quote_rollup` computes both live — same "compute, don't store" discipline as `bom_category_cost` (Section 59), applied here to sales data. `salesperson_daily_activity` replaces the live system's hand-assembled daily-report query the same way.

**Not built here:** OCR/business-card-scan handling (an integration concern — extracted fields land in these same columns either way), WhatsApp/email send logic (generated at the application layer from data already here, same deep-link pattern already used throughout this design), and real RLS (still the standing open item — this migration shapes the tables so the row-level rule from Section 68, "a salesperson sees only their own leads/quotes/customers unless Owner/Sales Head," can finally be written correctly once RLS itself is designed).

## 80. Service Desk — Tickets + AMC, Rebuilt as One Module (0037)

**The single biggest structural fix in this migration: one customer universe, not three.** The live system ran Service Desk's own synced `CUSTOMERS` tab (fed daily from three different external government/utility portal exports), Sales' separate `Clients` tab, and NRG SolarConnect's own `customers`/`projects` — three disagreeing pictures of the same real people. Three independent warranty calculators existed *because* of this fragmentation, each with a different rule and a different output shape (`docs/system-understanding-2026-08-16.md`, confirmed by reading all three implementations). Here there is exactly one `customers` table (Section 19) and one `projects` table for anything NRG installed — a service ticket references them directly. `customers.merged_into_customer_id` is added (same repair pattern as `materials`/`projects`, Sections 60/69) specifically because Service Desk's multi-source sync is what actually produces real duplicate customer records — the live system already had a manual dedup tool for exactly this reason.

**Warranty becomes one computed answer, not three.** `project_warranty_status` picks the ONE rule that actually gated real business logic in the live system (1 year if the project has a GEDA reference, else 5 years) and computes it live off `commissioning_reports.commissioning_date` (Section 37, already exists — no new install-date tracking needed). No confirmed commissioning date means the view genuinely says "unknown," never guesses either way — same "missing must never silently read as a specific value" discipline as `margin_data_complete` (Section 64), applied here to a warranty question instead of a cost one.

**A ticket can exist without a project.** AMC in particular serves third-party customers — systems NRG didn't itself install (`amc_quotes.customer_mode = 'third_party'`) — so `service_tickets.project_id` and `amc_quotes.project_id`/`amc_contracts.project_id` are all nullable; `customer_id` is the one thing always required.

**One photo/document storage convention, not two.** The live system had a config-anchored root folder for ticket photos but a search-by-name-with-no-anchor pattern for AMC photos/PDFs — confirmed to be a materially more fragile approach for AMC specifically. Here every photo is an ordinary `documents` row (Section 19, already nullable on `project_id` since Section 36/0009) linked through `ticket_documents`/`amc_visit_documents` join tables — one convention, everywhere.

**AMC contract status is computed, not stored — because the live version was silently dead.** The live `AMC_REGISTER` never transitioned a contract's status once its end date passed; nothing existed to move it to `'Expired'`. Rather than add a status column that would just be one more thing that can go stale, `amc_contract_status` computes `active`/`expired`/`cancelled` live from `start_date`/`end_date`/`cancelled_at` — it can never disagree with reality because there's nothing stored to disagree. `due_for_renewal` on the same view replaces the live daily 9am trigger that, despite having a real schedule and a UI badge, only ever wrote to an execution log and never actually notified anyone — confirmed dead-in-practice. Real renewal notification (email/WhatsApp) stays an application-layer follow-up; what changes here is that "which AMCs need attention" is now a real, live, trustworthy query rather than one nobody could rely on.

**The Commissioning Readings dead-stub gets a real destination, not a new table.** The live ticket UI has a full "Commissioning Readings" modal (DC/AC voltage, current, frequency, power) that never actually saved anywhere — confirmed, the form silently discarded everything typed into it. No new readings table here: `commissioning_reports` (Section 37/0010) already exists for exactly this data. The fix is wiring a commissioning-type ticket to write there, at the application layer — reusing infrastructure instead of adding a second, parallel place readings could live.

**Assignment rules become editable data.** `service_assignment_rules` replaces the live system's static complaint-type→engineer table, which was duplicated verbatim in server code and again in client-side JavaScript (confirmed real duplication) — one table, one place to change it.

**Not built here** (same standing list as every module so far): WhatsApp/email send logic, real payment-gateway integration (service/AMC payment confirmation stays staff-attested, matching `payment_receipts` elsewhere in this schema), and RLS itself — this migration shapes the tables so "an engineer sees their own assigned tickets, Owner/Service Head sees all" can finally be written correctly, it doesn't write that policy. Worth repeating here specifically: the live system's permission checks were confirmed almost entirely client-side theater — functions commented `// Admin only` with zero enforcement in their actual body. That gets fixed *by* building real RLS, not by anything in this migration alone.

## 81. Prospect CRM + Solar Bill Analyser (0038)

**The two safest, most self-contained remaining live tools** — confirmed by the migration plan's own risk tiering (`docs/sales-service-migration-plan.md`): neither shares a live spreadsheet with anything else, unlike the Quote/Follow-up pair (Section 79).

**Built to match what the data actually shows is used, not what the code merely implements.** Reading the live Prospect List spreadsheet directly (not just its code) found the elaborate 8-month drip-sequence/batch/stage-progression engine is dormant in practice: 3,690 companies, only 15 ever ran through real sequencing, all 180 scheduled touch-steps permanently stuck "Pending" because nothing marks them sent. Real activity (507 logged touches in the week reviewed) runs through a separate, simpler mechanism entirely. Building the elaborate dormant engine here would be over-building against direct evidence it isn't needed — so this migration builds `prospect_companies`/`prospect_hotels`/`prospect_hospitals`/`prospect_builder_groups`/`prospect_builder_projects`/`prospect_site_contacts` and one unified activity log, and explicitly does **not** build automated sequencing or batching. If real demand for that shows up later, it's a schema addition, not a redesign of what's here.

**One touch log, not two.** The live system ran an old "TOUCH LOG" mechanism nothing actually calls anymore alongside a newer "PROSPECT ACTIVITY" log that's what real usage runs through — the same class of fragmentation already fixed once in Section 79 (Quote/Follow-up's activity logging), fixed the same way here: `prospect_activity_log`, one table, one shape.

**Four separate prospect-entity tables, not one generic one.** Companies (factories), hotels, hospitals, and builder projects genuinely carry different real fields (a builder project's RERA completion date has no equivalent for a factory) — the live system already draws this distinction correctly across four different record types; collapsing them into one generic "prospect" table would lose information the source data actually has. `prospect_site_contacts` (the civil incharge/PMC/architect/consultant directory) uses a "pick exactly one of three FKs" check constraint rather than a nullable free-for-all, so a contact can never end up orphaned from every project type.

**The scraper review-queue shape survives even though the scraper doesn't.** `prospect_sources` keeps the "something proposes a prospect, a human approves or rejects it" pattern — the RERA/MCA scrapers themselves are confirmed pure stubs in the live system (they log a message and do nothing else), but the review-queue shape is the same real AI-suggests/human-confirms pattern used throughout this schema, and costs nothing to keep even with no real scraper behind it yet.

**`converted_lead_id`/`lead_id` are the real bridge into the sales pipeline.** Both a prospect entity and a bill-analysis report can point at a `leads` row (Section 79) once someone actually follows up — cold prospecting and lead-generation both feed the same pipeline the Quote/Follow-up module already owns, rather than existing as a parallel, disconnected universe.

**Solar Bill Analyser is the cleanest tool of the five read** — no dead code, no dormant automation found — so its schema is a straightforward transcription: `bill_analysis_settings`/`bill_analysis_rate_bands` (tenant-scoped sizing config, same reasoning as `quote_kw_band_rates`/`amc_rates`) and `bill_analysis_reports`, one row per analysed bill.

**Not built here:** the automated sequence/campaign/batch engine (confirmed dormant, above), a real RERA/MCA scraper implementation, OCR (application-layer, same as every other module), and RLS (same standing item as every module so far).

## 82. Client Engagement — Warranty Re-engagement Outreach, and a Mystery from Section 80 Closed (0039)

**The first of the six additional `nrg-cover`-hosted tools (Section 50/next-session item), fully source-read before designing anything, same discipline as the first five.** Turned out to be the smallest live tool found yet — two files, ~230 lines total, no HTML of its own (that lives in `nrg-cover`'s `nrg_client_engagement.html`). Confirmed to have **zero** warranty-expiry logic, zero WhatsApp template library, zero time triggers, and zero outbound calls of any kind (`UrlFetchApp` doesn't appear anywhere in its source) — it's a thin 3-endpoint read/append shim, and every piece of real intelligence lives entirely in the front-end.

**This closes a real, previously-flagged loose end.** Section 80's own deep read of Service Desk found `CE_LOG` — a tab in the shared spreadsheet with live, recent entries — referenced nowhere in any of Service Desk's 8 files, and flagged it as "an active parallel process... worth a follow-up investigation." This is that follow-up: Client Engagement's own header comment identifies the exact same spreadsheet as "the Service file (NRG Service Desk spreadsheet)," and its `logEngagement_` function is precisely what writes `CE_LOG`. `customer_engagement_log` is its direct replacement.

**Gated under the existing `service` entitlement, not its own new one — because the live system's own claimed isolation doesn't actually hold.** The live project's header comment boasts it's "DEDICATED, ISOLATED... shares NO code with Service Desk," and that's true at the code level — but it reads and writes the identical spreadsheet Service Desk owns, meaning it is **not** data-isolated, and a header rename in Service Desk's `CUSTOMERS` tab would silently blank fields here with no error (confirmed: the live code substitutes `''` on any header-lookup miss, never raises one). Section 78's entitlement design exists to model real boundaries, not aspirational ones — this one goes under `service` because that's what the data coupling actually is.

**Warranty math doesn't get reproduced here — it gets replaced with the answer that already exists.** The live tool recomputes warranty client-side, in the browser, from raw customer rows, on every page load — a fourth place the same calculation could drift, on top of the three Section 80 already found and fixed. This module's UI should simply query `project_warranty_status` (Section 80/0037) instead. The seasonal WhatsApp template library is presentation content, not schema — it can stay static UI copy, or become a small config table later if it ever needs to be admin-editable, neither of which blocks building this module now.

**A fifth confirmed no-auth live endpoint.** Same wide-open `ANYONE_ANONYMOUS` pattern as Quote Generator and Prospect List — full customer PII (name, phone, address, salesperson, kW, install date) readable by anyone with the URL, no token, no login. Not fixed here (schema-only round); adds to the running list of things real RLS needs to close.

## 83. Referral Network — Trade-Contact Partner CRM (0040)

**Located via the owner's own `nrg-cover` repo rather than Drive search.** `referral_network.html` (1164 lines) was read in full — a standalone CRM for external referral partners (electrical consultants, MEP, architects, PMCs, structural/civil, plumbing consultants, bath/ceramic shops, builders), with three tabs (contact list, a 30-day visit planner, and a WhatsApp broadcast tool) plus an AI business-card-scan-to-form feature. Its backend (Apps Script Web App v4, its own `SCRIPT_URL`) couldn't be located via Drive search, so this migration was first written from the frontend's full request/response contract alone — every column traced to a concrete `fetch()` call (`getContacts`, `addContact`/`editContact`, `toggleActive`, `logVisit`, `logWA`), nothing guessed. The owner then pasted the complete backend source directly (same unblock as Havells Quotes, Section 84), confirming every column and surfacing one real fact the frontend alone couldn't show: **the live sheet has no separate visit-log or WhatsApp-log storage at all** — `logVisit()`/`logWA()` just string-concatenate a bracketed line (`"[Visit dd-MMM-yy - CY] note text"`, `"[WA dd-MMM-yy - CY] template name"`) onto the end of one growing free-text Notes cell per contact, mixing hand-typed notes with an auto-appended activity trail. `referral_visit_log`/`referral_wa_log` are a deliberate upgrade, not a faithful reproduction, matching the same "turn a text blob into real rows" fix already applied to Sales Follow-up (Section 79) and Prospect List (Section 81). Importing history means parsing each contact's Notes cell for that bracket convention — each `[Visit ...]`/`[WA ...]` line becomes one log row, whatever's left over is what actually imports into the new `notes` column.

**Also confirms a sixth live no-auth endpoint** — same "Execute as Me / Anyone" deployment as Quote Generator, Prospect List, and Client Engagement. Full partner PII (name, phone, email, address) is readable *and writable* by anyone with the URL, no token, no login. Adds to the running list real RLS needs to close.

**The real integration point was already sitting in the Prospect CRM deep read (Section 81/0038), just not connected until now.** `syncSiteContactsToReferral_()` in Prospect List's `ProjectScript.js` writes automatically into this exact same spreadsheet (`1qph-OYqXcLq3YWaz5BJecQoFBlWge2A7zxAB6B7nTkI`) whenever a project/hotel/hospital site contact is added — civil incharge, PMC, architect, electrical/plumbing consultant, main contractor — deduped by stripped-digits mobile. That's why `referral_contacts.contact_type` lines up almost exactly with `prospect_site_contacts.role`: they are two views onto the same real population, one auto-fed, one hand-curated. `source_site_contact_id` (nullable FK to `prospect_site_contacts`) makes that link real.

**Its own module_entitlements key, `referral_network`** — functionally independent of every quoting module (a tenant can run this CRM with zero quoting modules enabled; a pure-solar tenant just never sets `product_interest` to `heat_pump`/`both`), so it earns a real boundary rather than riding on an existing key the way Client Engagement rode on `service` (Section 82).

**Compute, don't store, again**: `last_visited_at`/`last_wa_sent_at`/`visit_status` are a view (`referral_contact_status`), not columns — `MAX()` over two small append-only logs is trivial and can never drift, matching `customer_quote_rollup`/`project_warranty_status`/`amc_contract_status`. `visit_status`'s thresholds (30-day interval, 7-day due-soon window, never-visited = overdue) are copied exactly from the live UI's own `visitStatus()` function so the computed answer can never silently disagree with what the app shows.

**Confidence note, stated plainly**: the backend script itself was not read — only its complete client-side contract. If the owner later locates/pastes the actual Apps Script source (the way Havells Quotes' was unblocked, Section 84), it should be checked against this schema before going live, the same "read before you build" discipline as everywhere else in this document.

## 84. Havells Quotes (0041)

**Unblocked by the owner pasting the complete literal Apps Script source directly in chat** (`HHP-GAS-v5.1`, ~140 lines — `doPost`/`doGet`/`toNum`/`saveHHPQuote`/`generatePDF`/`generateHHPQuoteNo`/`getQuotes`/`jsonResp`) after its Drive script link returned "Requested entity was not found." Cross-checked against the live `HHP_QUOTES` sheet's real header row and real rows (via Drive full-text search) and against `havells_quote_v2.html` (the quoting frontend, read in full) for the Client Type / salesperson / product-catalog values the backend only ever receives as opaque strings. All three sources agree.

**A confirmed live production bug, now structurally impossible to repeat.** `saveHHPQuote()` always returns `{ ok: true, quoteNo, pdfUrl }`, even when PDF generation fails — on failure it sets `pdfUrl` to the literal string `'PDF generation failed: ' + err.message` and writes that into the sheet's "PDF URL" column. This isn't theoretical: real rows in the live sheet contain values like `'PDF generation failed: You do not have permission to call DriveApp.getRootFolder...'` sitting exactly where a Drive link should be — sales staff have no way to tell a quote has no PDF without reading the cell as English. `havells_quotes` splits this into two columns, `pdf_drive_file_id` and `pdf_generation_error` — success and failure can never again share one column.

**`havells_products`** is a small dedicated catalog table, not a bolt-on to `materials` (Section 19) — Havells units carry a manufacturer product code, a Split/All-in-One/Commercial type, and a capacity that's sometimes litres and sometimes kW, none of which fit materials' BOM-shaped columns without adding fields meaningless to every solar BOM row. No natural anchor-table FK, so `organization_id` direct, same as `quote_kw_band_rates` (Section 79) and Section 83's `referral_contacts`.

**`customer_id` reuses the shared `customers` anchor**, the same upgrade already made for solar Quotes (Section 79) — the live sheet has no customer-matching step at all, just raw name/phone/email/address per row, so every historical row will need one done at import time (`imported_from_legacy` flags it, same pattern as Section 79).

**Line items normalized, not the live "Products (JSON)" blob** — same upgrade as Quote Extras (Section 79). `discount_pct` is stored as entered, not reverse-derived — checked directly against real data (`mrp` vs. `unit_price` don't reconcile by any simple formula), so it's recorded as given rather than guessed at.

**Totals computed, not stored** (`havells_quote_totals` view) — verified line-by-line against real live rows that product subtotal + charges, ×1.18 for GST, is exactly the arithmetic the live sheet's own columns already agree with. GST is hardcoded at 18% to match the live "GST (18%)" header, flagged as the one number that should become configurable if the rate ever changes rather than silently baked in forever.

**Its own module_entitlements key, `havells_quotes`** — a genuinely independent live deployment (own spreadsheet, own Drive folder, own Railway PDF microservice) for a product line (Havells heat pumps) most white-label solar-EPC customers of this platform won't carry, same reasoning already applied to Quote Generator vs. Sales Follow-up (Section 79).

**`next_document_number('havells_quote', 'perpetual')`** — reuses the existing Section 37 sequence function as-is. The live `HHP-NNN` numbering never resets (confirmed by reading `generateHHPQuoteNo()`: a flat scan-for-max-and-increment across the sheet's whole history), so a constant `fiscal_year` value gets the same atomic, race-free numbering without forcing a reset the live series was never designed to have.

## 85. Tasks — Internal Team Task Manager + Grading (0042)

**Unblocked the same way as Havells Quotes and Referral Network**: the owner pasted the complete backend source directly, then a follow-up spreadsheet link (`NRG Tasks — TASKS Sheet Data`) confirmed the real 14-column header row and sample rows exactly matched what the backend's `appendRow()`/`setCol()` calls alone implied — no guessing needed. `nrg_tasks.html` (`nrg-cover`, read in full) supplied the one thing the backend never sees: `effectiveStatus()`, the A-D completion-timing grade formula, and the team leaderboard.

**Genuinely internal and NOT project-scoped** — confirmed by reading `createTask()`: nothing in the payload references a project. This is a general team task tracker a tenant could use standalone, so it gets its own `module_entitlements` key, `tasks`.

**A real value in the live data changed the design mid-build**: the sample sheet's `Assignee` column contains `"Office Team"` — not an individual — alongside real named staff. A hard FK to `employees` would have silently broken importing that exact row. `assignee_name`/`assignee_id` (raw text always captured, FK nullable and best-effort) mirrors the identical pattern already established for `leads.raw_name`/`customer_id` (Section 79) — the same real-world shape, a task assignee, showing up in a genuinely different module and getting the same fix.

**Everything computable is a view, not a column** — same discipline as every module before this one. `task_effective_status` reproduces the live `effectiveStatus()` exactly (a completed task is never overdue, no matter how late it finished; an incomplete one past due is overdue regardless of its stored status). `task_grade` reproduces `getGrade()`'s A/B/C/D formula exactly, including its zero-duration edge case (a task due the same day it was created) — verified the live JS's `NaN <= 0.85` resolving to `false` and falling through to grade B is exactly what Postgres's `nullif()`-guarded division produces too. `employee_task_performance` reproduces the live leaderboard's A=4/B=3/C=2/D=1 average-score-to-letter-grade aggregation, grouped by `assignee_name` rather than `assignee_id` specifically so a team label with no employee row still shows up correctly.

**A seventh confirmed no-auth live endpoint, and the worst of the seven found this session.** Every other one is reachable by anyone with the URL; this one's backend sets `Access-Control-Allow-Origin: *` explicitly, meaning it's callable cross-origin from any website's own JavaScript, not only from `nrg-cover` itself. Adds to the running list real RLS/auth needs to close — not fixed here, schema-only round.

## 86. Reference Finder — Find Comparable Installed Customers to Share as References (0043)

**Located directly in the cloned `nrg-cover` repo** (`nrg_reference_finder.html`, 549 lines, read in full) — the owner's own instruction was "check the github... I think the html file lives there," and it did, already sitting locally from the original repo clone, just not yet read line by line. No dedicated backend exists at all: the file's own comment says so — "Same Apps Script that serves Leads & Followup" — calling a new `?api=customers` query param on the exact Quote Generator/Sales Follow-up deployment already fully modeled (Section 79/0036).

**The "third spreadsheet" flagged during the very first Quote Generator deep-read turned out not to be new ground.** `getCustomers_()` was already known to read a separate spreadsheet ID (`FINAL_CLIENT_SHEET_ID`) outside the 14-tab shared sheet — re-fetching that ID now shows its real title is **"NRG Service Desk,"** and its `CE_LOG` tab is the exact tab already migrated as `customer_engagement_log` (Section 82/0039). So this is Service Desk's own `CUSTOMERS` tab — already `customers` (Section 2), `project_confirmed_bom` (Section 63/0024), and `commissioning_reports` (Section 37/0010) — not an unmodeled data source. **No new tables were needed**, only one real gap and one view.

**The real gap**: `customers` has `area` but nothing for the live system's real business-type taxonomy — Doctor, Hotel, Industry, School, Apartment, Residential, SPP Residential — read directly off the live page's own quick-filter pills and matching logic, not guessed. `projects.project_type` (`residential_subsidy`/`commercial_industrial`) is too coarse to answer "is this a doctor's clinic." `customers.business_type` (free text, same convention as `materials.category`) closes that gap.

**`customer_reference_directory`** is a view, computing everything the live page shows from data that already exists — total kW installed (summed across every commissioned project), and the "representative" system's area/panel-make/inverter-make/install-date, picked from that customer's SINGLE LARGEST install, not their most recent one — verified against `buildAgg()`'s own `_mx`-tracking logic line by line, which does exactly that. Panel make matches the confirmed exact category string `'Solar Panels'` (Section 59); inverter make matches loosely (`ilike '%inverter%'`) since no equally-confirmed exact string for that category has surfaced in any deep read yet — flagged rather than risking a silent miss.

**No new `module_entitlements` row.** Reference Finder introduces no new data and no new write path at all — pure read-only reporting over data that exists the moment a tenant has commissioned projects. Rides on the existing `sales_followup` key (the live page's own eyebrow label is "Sales Help"), same reasoning as Client Engagement riding on `service` (Section 82) rather than manufacturing a boundary the real coupling doesn't have.

**This closes out the full six-tool scope expansion from Section 50** — every `nrg-cover`-hosted tool identified that round (Leads, Tasks, Client Engagement, Reference Finder, Havells Quotes, Referral Network) now has a designed Postgres replacement.

## 87. Message Templates — One Durable, Admin-Editable WhatsApp Template Library (0044)

**Direct owner correction, and a fair one.** Every module built this session had real preset WhatsApp messages, and every one of those prior migrations said some version of "template text is UI copy, not schema" and left the actual wording sitting only in live Apps Script/HTML source. The owner flagged this directly: if a live tool is ever retired before its replacement UI exists, that wording — real, tuned, working copy — is gone for good. This migration goes back through every source touched this session (the raw Apps Script manifests saved during the original deep-reads, plus `nrg_followup_dashboard.html`, `nrg_client_engagement.html`, `nrg_leads.html`, `havells_quote_v2.html`, `referral_network.html`, and Service Desk's `code.js`/`amc.js`, re-read directly) and copies **47 real message bodies, verbatim**, into one permanent table.

**The owner also gave a concrete architecture for how these get sent** — every module splits into exactly two channels: `individual_wa_me` (Sales Follow-up, Service, Leads, Quote Generator, Havells Quotes, Tasks — output must come from the salesperson's/service person's own phone, a wa.me deep link they personally tap send on) and `bulk_marketing` (Client, Referral Network, and Prospect Intelligence engagement — general stay-in-touch/education messaging, meant for a single-point bulk WhatsApp marketing tool, name not yet committed to). Two modules get **reclassified** from how their live tools work today: Client Engagement and Referral Network both use `wa.me` live, but both are exactly the "not a specific deal" pattern the owner just defined — moved to `bulk_marketing` here as a deliberate forward decision, called out explicitly since it's a real behavior change, not a bug fix.

**Genuinely new, deliberately left empty**: Prospect Intelligence (companies/hotels/hospitals) is the owner's third audience, and no live tool has ever built bulk messaging for it — `prospect_activity_log` (Section 81) only ever logged individual touches. No template rows are seeded for it here; writing fabricated marketing copy would break the exact "verbatim from a real source, never invented" discipline every other line of this migration follows. The schema already accepts it (`module_key = 'prospect_crm'`, `channel = 'bulk_marketing'`) — the owner's team fills it in through the same table, whenever the copy is ready.

**Editable "as we go," per a second explicit owner instruction** — `message_templates` is an ordinary table, not code. Updating a message going forward is a `UPDATE`/future admin-UI action, never a redeploy, directly satisfying "there should be a way where I could access these... and make changes to them easily."

**Attachments and custom messages, the owner's third explicit ask.** Every send-side log (`customer_activity_log`, `ticket_timeline`, `referral_wa_log`, `customer_engagement_log`, and the brand-new `prospect_engagement_log`) gained `template_id` (which preset, if any), `custom_text` (what was typed instead, when nothing preset fit), and `attachment_drive_file_id`/`attachment_type`/`attachment_external_url` (photo/video/brochure/link). `bulk_provider`/`bulk_external_ref` are added only where `bulk_marketing` sends actually happen — a provider-agnostic hook so whichever bulk WhatsApp platform is eventually adopted just populates two columns, no schema change needed then.

## 88. Per-Employee Module Access (Not Fixed Teams), and the employees/auth Gap Finally Closed (0045)

**Two direct owner instructions, arriving back to back, that turn out to be the same underlying gap.** First: team/login/password/module-access should live in one editable place, module-wise, "rather than have a set sales team or a purchase team" — because a real person can need Sales + Service, or Purchase + a bit of Tasks, and Section 48's own five-bucket role-to-screen table can't express that overlap. Second, immediately after: "some of the modules, there is no login check." Both point at the same root cause — the "known architectural gap" flagged all the way back in Section 40 of the very first build brief and never actually closed: `employees` has never had any link to a real identity provider, so there has never been anything for a login gate to check against.

**`employees.auth_user_id`** links to Supabase Auth (or an equivalent provider) — deliberately **no password column**. This is the secure version of "login IDs and passwords," not a partial answer to it: password hashing, sessions, and resets are the identity provider's job, the exact same "never store secrets in a plain column" rule already stated in Section 2, applied to the one secret that matters most. `employees.login_id` is the separate, human-facing piece the owner actually needs for the "sheet" — a short staff code (the same CY/RC/AP/... codes already seen live across Tasks, Referral Network, and Sales Follow-up) an admin can read and match to a name.

**`employee_module_access`** — one row per (employee, module), zero fixed categories, admin adds or removes a row to grant or revoke a module. Deliberately the same shape as `module_entitlements` (Section 78) one level down: that table answers "does this tenant have Service at all," this one answers "does this specific person, within a tenant that already has it, get to open it." No `organization_id` column — inherited transitively through `employee_id`, same anchor-table rule as everywhere else. `employees.role` (free text, Section 2) is **not replaced** — it still drives the narrower, already-decided rules like Owner-only cost/margin (Section 39) and pipeline-project row visibility (Section 68); module access is a coarser, complementary question, not a competing mechanism.

**Stated plainly what this does and doesn't close.** It makes real per-module login enforcement buildable — the identity link and the access list both now exist for a route guard or an RLS policy to check. It does not itself add that enforcement (still the standing item, Next Session #44) — same honest boundary already drawn for the multi-tenant foundation. It also does not close the specific no-auth holes already confirmed live this session (Quote Generator, elec_bill's `/ocr-bill`, Prospect List, Client Engagement, Referral Network, Tasks) — those run on the old Apps Script stack entirely outside this schema, and stay open until that code is patched directly or that traffic is actually migrated. What this guarantees is that the new system doesn't get built the same way a second time.

**`employees.is_owner`/`is_admin`** — added the same round, prompted directly by the owner's very next message: an "admin assistant" other than the owner needs to review/edit every WhatsApp template, control module access for everyone else, and see financial data — which the owner explicitly wants limited to "only me or the admin." This is also the first time "Owner" itself becomes a real, checkable column rather than free-text `role` matching — every prior Owner-only gate in this schema (cost/margin visibility, Section 39/63/64; pipeline-project pricing, Section 68) has only ever been described in prose. Going forward all of it reads as one consistent rule, `is_owner or is_admin`: editing `message_templates`, granting/revoking another employee's `employee_module_access`, and every financial/margin visibility gate already built. Deliberately one flat boolean, not a finer three-way split — the owner described one bundled admin capability, not separately grantable ones; a real need to split it further (someone who edits templates but shouldn't see margin, say) is a genuine future refinement, not something to guess at now.
...
21. ~~`materials.make`~~ — done, see Section 40 (brand-distinct stock, e.g. Adani vs. Waree at the same wattage — a named real gap where Sales couldn't see what was already on the shelf). Flagged as a permissions note too: Sales needs read access to stock levels even though they do little document processing in NRG SolarConnect itself.
...
38. ~~Editable pipeline BOM, and the Owner/Sales/Service split refined~~ — done, see Section 73. No schema change — reuses `extraction_status`/`confirmed_by` and `documents.uploaded_by`. Corrects Section 68: pipeline pricing is Owner-only, not Owner/Sales Head — that pairing was the row-visibility rule, a different axis.
39. ~~One-click signed payment receipt generation~~ — done, see Section 74 (migration 0033). Same generated-document pattern as Delivery Challans/Commissioning Reports; `payment_receipts` already has everything needed to auto-populate it.
40. ~~Pipeline BOM needs Rate editable alongside Quantity, missing line items addable, Transportation/Fabricator/Electrical as overridable numbers~~ — done, see Section 75 (migration 0034). Rate and missing-line-items needed no schema (`bom_items.planned_rate` and free-form row insertion already existed); `boms.estimated_transportation_cost`/`estimated_fabricator_cost`/`estimated_electrical_contractor_cost` are the one real addition, `coalesce`d over the size-band default in a recreated `pipeline_project_margin_estimate`. Concrete result on Bansal Ceramics: the four previously-missing line items flip estimated margin from +11.92% to −3.08% — the size-band estimate was silently incomplete, not wrong.
41. ~~Generated documents need NRG's real letterhead, not a placeholder~~ — done, see Section 76. No schema — a mockup/template correction, checked directly against real NRG documents (Quote, Debit Note, Delivery Challan, Sales Invoice). Real registered address/GSTIN now used throughout instead of a placeholder GSTIN. Flagging for the real build: embed NRG's actual logo file, not the SVG approximation built here.
42. ~~Renamed ProjectPulse → NRG SolarConnect, "Projects" pill on nrg-cover's existing home screen instead of a new front door~~ — done, see Section 77. No schema. Real, separate, larger initiative now underway alongside this document: migrating NRG's live Sales/Service Apps Script + Sheets stack (Service Desk, Quote Generator, Sales Follow-up, Prospect List, Solar Bill Analyser) onto this same Postgres foundation — tracked as its own migration plan, phased and reversible, starting with a baseline audit of every historical quote/report Drive link before anything live is touched.
43. ~~Multi-tenant foundation — organizations + module_entitlements, organization_id on the five anchor tables~~ — done, see Section 78 (migration 0035). Built ahead of the Sales/Service tables specifically so every new table from here on is tenant-scoped from its first migration, not retrofitted later. RLS enforcement itself is still open — same standing item 4 below.
44. **New, high priority**: real RLS policies enforcing organization_id isolation, now that the column exists everywhere it needs to (Section 78). This was always going to be needed (item 4); Section 78 is what finally makes it buildable correctly.
45. ~~Quote Generator + Sales Follow-up schema~~ — done, see Section 79 (migration 0036): `leads`, `customer_pipeline`, `quotes`, `quote_extras`, `quote_panel_rates`, `quote_kw_band_rates`, `customer_activity_log`, `sales_targets`, plus `customer_quote_rollup`/`salesperson_daily_activity` views. Still open, next: Service Desk (tickets + AMC), Prospect List, Solar Bill Analyser as their own modules, each gated by module_entitlements (Section 78). Full architecture already understood end-to-end, see `docs/system-understanding-2026-08-16.md` and `docs/sales-service-migration-plan.md`. Two live security gaps still flagged as worth fixing independent of this build: Quote Generator has no authentication at all today, and a two-stage WhatsApp-approval device auth (already built and working in `nrg-cover`, via `gas-verify.gs`) was never wired into it.
46. Mockup screens — deliberately deferred, per the owner's explicit call: build every Sales/Service module's schema first, module by module, do all the UI together afterward rather than interleaving it. Covers Sections 79 and 80 so far.
47. ~~Service Desk (tickets + AMC) schema~~ — done, see Section 80 (migration 0037): `service_tickets`, `ticket_timeline`, `ticket_documents`, `service_assignment_rules`, `service_quotes`/`service_quote_items`, `amc_rates`, `amc_quotes`, `amc_contracts`, `amc_visits`, `amc_visit_documents`, `customers.merged_into_customer_id`, plus `project_warranty_status`/`amc_contract_status` views.
48. Real RLS — now the standing item across five modules' worth of tables (Sections 68/79/80/81), not just one. Also still open from Section 79: the ordinary per-kW quote discount's server-side cap (currently client-JS-only in the live system, ₹4,000/kW) belongs with the RLS design, not schema.
49. ~~Prospect CRM + Solar Bill Analyser schema~~ — done, see Section 81 (migration 0038). All five of the originally-scoped live GAS tools now have a designed Postgres replacement (Sections 79-81). Confirmed the elaborate Prospect List sequence/batch engine dormant in the live data and built to match actual usage instead — see Section 81.
50. **Scope confirmed expanded, per explicit instruction**: bring everything in `nrg-cover` into NRG SolarConnect too, not just the five originally-scoped GAS projects. Six more tools were found hardcoding their own dedicated GAS backend during the `nrg-cover` deep read: Leads, Tasks, Client Engagement, Reference Finder, Havells Quotes, Referral Network. Of these: **Leads turned out not to be a separate module at all** — confirmed during the Quote Generator read (Section 79) that `addLead_`/`getOpenLeads_` already live inside that same project; `nrg-cover`'s Leads page just calls the same `QUOTE_GEN_EXEC` deployment. **Client Engagement** is done, see Section 82. **Havells Quotes** is done, see Section 84 — unblocked by the owner pasting its complete source directly after its Drive link failed. **Referral Network** is done, see Section 83 — its spreadsheet was locatable (it's the same `REFERRAL_SHEET_ID` constant found during the Prospect CRM read), its full frontend contract was read in `nrg-cover`, and its backend source (not locatable via Drive search) was subsequently pasted directly by the owner, the same way Havells Quotes was unblocked — the lower-confidence flag is now resolved, both remaining "extra" tools have full source-level confirmation. **Tasks** is done, see Section 85 — same unblock as the other two, owner pasted the backend source directly, then a spreadsheet link confirmed the exact column layout. **Reference Finder** is done, see Section 86 — found directly in the already-cloned `nrg-cover` repo (the owner pointed at GitHub directly rather than Drive), and turned out to need almost no new schema at all: one new `customers.business_type` column plus one view, since its data all traces back to tables/views already built for Service Desk and the cost-benchmark reports. **All six tools from this scope expansion are now done.** Do not schema-design these from `nrg-cover`'s client-side call shapes alone without flagging that as materially lower-confidence than everything else in this document — same "read before you build" discipline as the rest of this handover.
