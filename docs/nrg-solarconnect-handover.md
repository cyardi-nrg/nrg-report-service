...
47. ~~Service Desk (tickets + AMC) schema~~ — done, see Section 80 (migration 0037): `service_tickets`, `ticket_timeline`, `ticket_documents`, `service_assignment_rules`, `service_quotes`/`service_quote_items`, `amc_rates`, `amc_quotes`, `amc_contracts`, `amc_visits`, `amc_visit_documents`, `customers.merged_into_customer_id`, plus `project_warranty_status`/`amc_contract_status` views. Still open, next: Prospect List and Solar Bill Analyser as their own modules — both confirmed the safest, most self-contained remaining pieces (`docs/sales-service-migration-plan.md` risk tiering).
48. Real RLS — now the standing item across four modules' worth of tables (Sections 68/79/80), not just one. Also still open from Section 79: the ordinary per-kW quote discount's server-side cap (currently client-JS-only in the live system, ₹4,000/kW) belongs with the RLS design, not schema.
