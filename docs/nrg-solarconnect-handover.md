...
50. **Scope confirmed expanded, per explicit instruction**: bring everything in `nrg-cover` into NRG SolarConnect too, not just the five originally-scoped GAS projects. Six more tools were found hardcoding their own dedicated GAS backend during the `nrg-cover` deep read: Leads, Tasks, Client Engagement, Reference Finder, Havells Quotes, Referral Network. Of these: **Leads turned out not to be a separate module at all** — confirmed during the Quote Generator read (Section 79) that `addLead_`/`getOpenLeads_` already live inside that same project; `nrg-cover`'s Leads page just calls the same `QUOTE_GEN_EXEC` deployment. **Client Engagement** is done, see Section 82. **Still open, and not yet locatable**: Tasks, Reference Finder, Havells Quotes, Referral Network — none turned up in a Drive search by title or content the way the other projects did; they're most likely Apps Script bound directly to their own spreadsheets rather than standalone script files, which needs the owner to point at the actual spreadsheet (or its share link) rather than a title search. Do not schema-design these from `nrg-cover`'s client-side call shapes alone without flagging that as materially lower-confidence than everything else in this document — same "read before you build" discipline as the rest of this handover.
